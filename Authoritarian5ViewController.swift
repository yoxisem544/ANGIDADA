...
class Authoritarian5ViewController: UIViewController {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "15 to 16" {
            let vc = segue.destinationViewController as! BenevolentLeadership1ViewController
            vc.user = user
        }
    }
    ...
    @IBAction func nextClicked(sender: AnyObject) {
        performSegueWithIdentifier("15 to 16", sender: user)
    }
}

extension Authoritarian5ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        user.authoritarianLeadershipQuestion5 = fearLevel[row]
    }
}