...
class AuthoriarianLeadershipRecieved4ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "午間問卷".localized + " (4/10)"
        var backbutton = UIBarButtonItem(title: "4/10", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.authoritarianLeadershipReceived4 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! AuthoriarianLeadershipRecieved5ViewController
...
extension AuthoriarianLeadershipRecieved4ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.authoritarianLeadershipReceived4 = fearLevel[row]
    }
}