...
class AuthoriarianLeadershipRecieved2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "午間問卷".localized + " (2/10)"
        var backbutton = UIBarButtonItem(title: "2/10", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.authoritarianLeadershipReceived2 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! AuthoriarianLeadershipRecieved3ViewController
...
extension AuthoriarianLeadershipRecieved2ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.authoritarianLeadershipReceived2 = fearLevel[row]
    }
}