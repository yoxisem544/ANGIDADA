...
class AuthoriarianLeadershipRecieved1ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "午間問卷".localized + " (1/10)"
        var backbutton = UIBarButtonItem(title: "1/10", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.authoritarianLeadershipReceived1 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! AuthoriarianLeadershipRecieved2ViewController
...
extension AuthoriarianLeadershipRecieved1ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.authoritarianLeadershipReceived1 = fearLevel[row]
    }
}