...
class ExpectedQuantativeJobDemand4ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "早晨問卷".localized + " (4/20)"
        var backbutton = UIBarButtonItem(title: "4/20", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.expectedQuantitativeJobDemand4 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! ExpectedQuantativeJobDemand5ViewController
...
extension ExpectedQuantativeJobDemand4ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.expectedQuantitativeJobDemand4 = fearLevel[row]
    }
}