...
class Intensity3ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "初始問卷".localized + " (39/72)"
        var backbutton = UIBarButtonItem(title: "39/72", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        user.intensityOfSupervisorSubordinateInteractionQuestion3 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! Intensity4ViewController
            vc.user = user
        }
    }
...
extension Intensity3ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        user.intensityOfSupervisorSubordinateInteractionQuestion3 = fearLevel[row]
    }
}