...
class PositiveAffect8ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "晚間問卷".localized + " (8/29)"
        var backbutton = UIBarButtonItem(title: "8/29", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.positiveAffect8 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! PositiveAffect9ViewController
...
extension PositiveAffect8ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.positiveAffect8 = fearLevel[row]
    }
}