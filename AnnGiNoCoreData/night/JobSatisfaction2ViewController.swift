...
class JobSatisfaction2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "晚間問卷".localized + " (22/29)"
        var backbutton = UIBarButtonItem(title: "22/29", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.jobSatisfaction2 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! JobSatisfaction3ViewController
...
extension JobSatisfaction2ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.jobSatisfaction2 = fearLevel[row]
    }
}