...
class NegativeAffect4ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = "晚間問卷".localized + " (14/29)"
        var backbutton = UIBarButtonItem(title: "14/29", style: UIBarButtonItemStyle.Done, target: nil, action: nil)
        self.navigationItem.backBarButtonItem = backbutton
        // Do any additional setup after loading the view.
        fearLevelPickerView.selectRow(0, inComponent: 0, animated: true)
        questionare.negativeAffect4 = fearLevel[0]
    }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! NegativeAffect5ViewController
...
extension NegativeAffect4ViewController : UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        print(fearLevel[row])
        questionare.negativeAffect4 = fearLevel[row]
    }
}