...
class TelInfoViewController: UIViewController {
    ...
    @IBAction func nextClicked() {
        if contentTextField.text != "" {
            user.userPhone = contentTextField.text
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "next" {
            let vc = segue.destinationViewController as! EMAILInfoViewController
            vc.user = user
        }
    }
...
extension TelInfoViewController : UITextFieldDelegate {
